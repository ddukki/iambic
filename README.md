...
[![npm version](https://img.shields.io/npm/v/iambic.svg)](https://www.npmjs.com/package/iambic)
[![CI](https://github.com/ddukki/iambic/actions/workflows/ci.yml/badge.svg)](https://github.com/ddukki/iambic/actions/workflows/ci.yml)
...
`exportFile()` generates standalone HTML documents with a **Content Security Policy** meta tag:

```
default-src 'self'; img-src 'self'; style-src 'unsafe-inline'; script-src 'none'
```

This blocks inline scripts, external script hosts, and restricts image sources. When `allowExternalImages: true` is passed, `img-src` is relaxed to `*`.

### Image Backgrounds

Image backgrounds (`{ type: 'image', url: '...' }`) are **blocked by default** to prevent tracking pixels and SSRF. Pass `{ allowExternalImages: true }` to `render()` or `exportFile()` to enable them.

```ts
// Image backgrounds dropped:
render(poem)

// Image backgrounds rendered:
render(poem, { allowExternalImages: true })
```

### Input Sanitization

All user-controlled values (colors, URLs, gradient angles, dimensions) are sanitized before being embedded in CSS. Characters that break CSS context (`;`, `"`, `'`, `{`, `}`, etc.) are stripped. Fragment text is HTML-escaped.
...
Options: `RenderOptions` — `{ padding?: number, allowExternalImages?: boolean }`

### `exportFile(poem, options?)`

Returns a complete self-contained HTML document with Content Security Policy.
...
### `renderPoemHTML(poem, options?)`

Render without auto-normalizing (poem must already be normalized).

```ts
import { renderPoemHTML } from 'iambic'

const html = renderPoemHTML(normalizedPoem, { padding: 60 })
```

### `renderFullDocument(poem, options?)`

Returns a full HTML document with `<!DOCTYPE html>`, CSP meta tag, meta viewport, centered layout.
...
interface RenderOptions {
  padding?: number       // Content padding in px (default: 40)
  allowExternalImages?: boolean  // Render image backgrounds (default: false)
}
```
